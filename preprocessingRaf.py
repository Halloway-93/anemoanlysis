...
main_dir = '/Users/mango/oueld.h/dataRaphaelle'
os.chdir(main_dir)
...
# %% ANEMO parameters
screen_width_px = 1024  # px
screen_height_px = 768  # px
screen_width_cm = 44.7  # cm
...
for idxSub, sub in enumerate(subjects):
    ...
    try:
        h5_file = "{sub}_posFilter.h5".format(sub=sub)
        h5_rawfile = "{sub}_rawData.h5".format(sub=sub)
        h5_qcfile = "{sub}_qualityControl.h5".format(sub=sub)

        paramsRaw = []
        qualityCtrl = []
        paramsSub = []

        dataFile = "{main_dir}/{sub}.asc".format(
            main_dir=main_dir, sub=sub
        )  # VANESSA lab
        tgDirFile = "{main_dir}/eyelink_trials.csv".format(
            main_dir=main_dir
        )  # VANESSA lab

        outputFolder_plots = "plots/"
        fitPDFFile = "{plotFolder}{sub}_fit.pdf".format(
            plotFolder=outputFolder_plots, sub=sub        )
        ...
        # testing
        nanOverallFile = "".join(
            [outputFolder_plots, "qc/", sub, "_nanOverall.pdf"]
        )
        nanOnsetFile = "".join(
            [outputFolder_plots, "qc/", sub, "_nanOnset.pdf"]
        )
        nanSequenceFile = "".join(
            [outputFolder_plots, "qc/", sub, "_nanSequence.pdf"]
        )
        ...
        data = read_edf(dataFile, start="stationary target", stop="motion_offset")

        # tg_up = pd.read_csv(tgDirFile, sep="\t")["trial_color_UP"]

        tg_dir = pd.read_csv(tgDirFile)["targetDir"]

        # change directions from 0/1 diagonals to -1/1
        # param_exp["dir_target"] = [x if x == 1 else -1 for x in tg_dir]
        param_exp["dir_target"] = [1 if x == "right" else -1 for x in tg_dir]
        ...
        for trial in list(range(param_exp["N_trials"])):
            print("Trial {0}, sub {1}".format(trial, sub))
            ...
            if len(data[trial]["x"]) and len(data[trial]["y"]):
                ...
                pos_deg_x = A.data_deg(
                    ...
                    before_sacc=sacc_params["before_sacc"],
                    after_sacc=sacc_params["after_sacc"],
                    filt=None,
                    cutoff=30,
                    sample_rate=1000,
                )
                ...
                if (
                    ...
                    if equation == "sigmoid":
                        ...
                        param_fit, inde_var = Fit.generation_param_fit(
                            ...
                            trackertime=time_x,
                            TargetOn=time_x[newTargetOnset],
                            StimulusOf=time_x[0],
                            saccades=new_saccades,
