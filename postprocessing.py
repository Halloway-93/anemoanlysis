...
subjects   = ['sub-01', 'sub-02', 'sub-03', 'sub-04', 'sub-05', 'sub-06','sub-07', 
            'sub-08', 'sub-10', 'sub-11', 'sub-12', 'sub-13'
...
keys2save = ['subject','condition', 'trial', 'trialType', 'direction', 'time_x', 
            'posDeg_x', 'velocity_x',]

float_keys = ['posDeg_x', 'velocity_x', ]
    ...
    for cond in conditions:
        ...
        try:
            ...
            for index, row in temp.iterrows():
                ...
                newData = np.vstack((subj, condi,trial,trialTp,tgdir,row['time_x'],
                            row['posDeg_x'],row['velocity_x'])).T
...
keys = ['cond', 'trial', 'target_dir', 'trialType', 
        # 'trialTgUP',
        'aSPon', 
        'aSPv', 
        'SPacc',
...
for sub in subjects:
    ...
    try: tempDF.drop(['velocity_y', 'time'], axis=1, inplace=True)
    except: pass
