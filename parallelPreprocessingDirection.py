...
def process_subject_session(main_dir, sub, session, sacc_params, px_per_deg, param_exp, 
                          time_sup=10, showPlots=0, manualCheck=0, 
                          equation="sigmoid", allow_baseline=True, 
                          allow_horizontalShift=True, allow_acceleration=False):
    """
    Process a single subject-session pair
    
    Returns:
        dict: Processing results or None if failed
    """
    print(f"Processing Subject: {sub}, Session: {session}")
    ...
    try:
        ...
        # Determine start event based on directory type (imposed or voluntary)
        ...
        # Different column names based on experiment type
        if "imposed" in main_dir.lower():
        ...
        for trial in list(range(param_exp["N_trials"])):
            ...
            if len(data[trial]["x"]) and len(data[trial]["y"]):
                data[trial]["y"] = param_exp["screen_height"] - data[trial]["y"]
        ...
        return {"subject": sub, "session": session, "status": "success"}

    except Exception as e:
        print(f"Error processing {sub}, session {session}: {e}")
        traceback.print_exc()
        return {"subject": sub, "session": session, "status": "failed", "error": str(e)}


def main():
    """Main function to process all subjects and sessions in parallel"""
    ...
    # Setting up parameters that are common for all jobs
    ...
    showPlots = 0  # Set to 1 to show plots
    manualCheck = 0  # Set to 1 for manual checking
    equation = "sigmoid"
    allow_baseline, allow_horizontalShift, allow_acceleration = True, True, False
    
    all_jobs = []  # Will store all jobs to run in parallel
    
    # Loop through each directory
    for main_dir in directories:
        print(f"Processing directory: {main_dir}")
        
        # Set up experimental parameters based on directory
        if "imposed" in main_dir.lower():
            param_exp = {  # Imposed
                "N_trials": 1,
                "N_blocks": 1,
