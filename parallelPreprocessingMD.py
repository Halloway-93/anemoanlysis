...
showPlots = 0  # Whether to show and save plots for every trial
manualCheck = 0  # Whether to manually check trials

equation = "sigmoid"
allow_baseline, allow_horizontalShift, allow_acceleration = True, True, False

# Define a function to process a single subject/condition pair
def process_subject_condition(sub, cond):
    ...
    try:
        ...
        try:
            ...
            pass
        # make figure folder for quality control
        try:
        ...
        current_param_exp = param_exp.copy()  # Create a local copy
        current_param_exp["dir_target"] = [x if x == 1 else -1 for x in motionSecondSegment]
        current_param_exp["N_trials"] = len(data)

        # creates an ANEMO instance
        A = ANEMO(current_param_exp)
        Fit = A.Fit(current_param_exp)
        ...
        # Process each trial
        for trial in list(range(current_param_exp["N_trials"])):
            print(f"Processing Trial {trial}, cond {cond}, sub {sub}")
            ...
            if len(data[trial]["x"]) and len(data[trial]["y"]):
                ...
                type_dir2 = "R" if current_param_exp["dir_target"][trial] == 1 else "L"
                ...
                sDict = {
                    ...
                    "direction": current_param_exp["dir_target"][trial],
                    "time": time,
                    "posPxl_x": pos_x,
                    "posPxl_y": pos_y,
                    "posDeg_x": pos_deg_x,
                ...
                if (
                    ...
                    if equation == "sigmoid":
                        ...
                        result_x = Fit.Fit_trial(
                            vel_x,
                            equation="fct_velocity_sigmo",
                            dir_target=int(current_param_exp["dir_target"][trial]),
                            trackertime=time,
                            TargetOn=0,
                            StimulusOf=time[0],
                            saccades=new_saccades,
        ...
        print(f"Successfully processed and saved data for {sub}, condition {cond}")
        
        del abc, paramsRaw,paramsSub,qualityCtrl

        return f"Completed {sub}, {cond}"

    except Exception as e:
        print(f"Error! \n Couldn't process {sub}, condition {cond}")
        traceback.print_exc()
        return f"Failed {sub}, {cond}: {str(e)}"

# Create a list of all subject-condition pairs to process
subject_condition_pairs = []
for sub in subjects:
    for cond in conds_by_sub[sub]:
        subject_condition_pairs.append((sub, cond))
