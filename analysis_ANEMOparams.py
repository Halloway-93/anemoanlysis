...
lmm_dir = "~/Documents/PhD/Experiments/data/dataHamza/LMM"
...
keys2plot = [['aSPon', 300, 'Anticipation Onset', 'ms'],
...
for k in keys2plot: #for each variable to plot
    ...
    sns.violinplot(x='pR-Red', y=k[0], hue='trial_color',
                        data=data2plot, palette=colormap, saturation=1,
                        bw=0.3, cut=0, size=3, aspect=1, 
                        linewidth=1, split=True, inner = "quartile")
    ...
    for sub in subjects:
        ...
        sns.violinplot(x='pR-Red', y=k[0], hue='trial_color',
                        data=data2plot[data2plot['sub']==sub], palette=colormap, saturation=1,
                        bw=0.3, cut=0, size=3, aspect=1, 
                        linewidth=1, split=True, inner = "quartile")
...
# export data for LMM
dataSub.to_csv(f'{lmm_dir}/dataANEMO_allSubs_activeColorCP.csv')
...
for k in keys2plot: #for each variable to plot
    ...
    sns.violinplot(x='pR-Red', y=k[0], hue='trial_color', hue_order=['Green', 'Red'],
                        data=data2plot[data2plot['target_dir']==-1], palette=colormap, saturation=1,
                        bw=0.3, cut=0, size=3, aspect=1, 
                        linewidth=1, split=True, inner = "quartile")
    # ax.set_xticklabels(labels=list(condList.values()))
    plt.xlabel('P(Right|Red)')
...
xAxis = np.array([-.25,0,.25])
xAxis = np.array([.25,.5,.75])
...
for p in anticipParams:
    ...
    for sub in subjects:
        ...
        pRRed25_redTg   = (s_pR_Col+pR_Col)*x[0] + (s_trialCol+trialCol)*0 + (pR_Col_tCol)*x[0]*0 + s_intercept + intercept
        pRRed50_redTg   = (s_pR_Col+pR_Col)*x[1] + (s_trialCol+trialCol)*0 + (pR_Col_tCol)*x[1]*0 + s_intercept + intercept
        pRRed75_redTg   = (s_pR_Col+pR_Col)*x[2] + (s_trialCol+trialCol)*0 + (pR_Col_tCol)*x[2]*0 + s_intercept + intercept

        pRRed25_greenTg = (s_pR_Col+pR_Col)*x[0] + (s_trialCol+trialCol)*1 + (pR_Col_tCol)*x[0]*1 + s_intercept + intercept
        pRRed50_greenTg = (s_pR_Col+pR_Col)*x[1] + (s_trialCol+trialCol)*1 + (pR_Col_tCol)*x[1]*1 + s_intercept + intercept
        pRRed75_greenTg = (s_pR_Col+pR_Col)*x[2] + (s_trialCol+trialCol)*1 + (pR_Col_tCol)*x[2]*1 + s_intercept + intercept

        ax.plot(np.array([.25, .5, .75])+.05, [pRRed25_redTg, pRRed50_redTg, pRRed75_redTg], color=colormap['Red'], alpha=.1)
        ax.plot(np.array([.25, .5, .75])-.05, [pRRed25_greenTg, pRRed50_greenTg, pRRed75_greenTg], color=colormap['Green'], alpha=.2)

    sns.swarmplot(data=anticipData, x="pR-Red", y=p[0], hue="trial_color", palette=colormap, dodge=True, legend=False, size=4, ax=ax2)
    sns.boxplot(data=anticipData, x="pR-Red", y=p[0], hue="trial_color",  palette=colormap, dodge=True, ax=ax2,
                width=.85,
                showfliers=False, showmeans=False, 
                meanprops=dict(marker='o', markerfacecolor='white', markeredgecolor='none', markersize=3, zorder=3),
